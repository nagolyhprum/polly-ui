...
import scrollable from 'polly-ui/plugins/scrollable'
describe('scrollable', () => {
  it('repositions', () => {
    const view = View({
      parent: {
  ...
  it('scrolls', () => {
    ...
    view.events.call('onMouseMove', ({ x: 0, y: 0 }))
    expect(view.scrollX).toEqual(0)
    expect(view.scrollY).toEqual(0)
    view.events.call('onMouseDown', ({ x: 0, y: 0 }))
    expect(view.scrollX).toEqual(0)
    expect(view.scrollY).toEqual(0)
    view.events.call('onMouseMove', ({ x: -1.2, y: -1.2 }))
    expect(view.scrollX).toEqual(-1.2)
    expect(view.scrollY).toEqual(-1.2)
    view.events.call('onMouseUp')
    // test friction
    screen.interval()
